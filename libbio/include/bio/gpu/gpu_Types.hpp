...
namespace bio::gpu {
    ...
    enum class BinderErrorCode : i32 {
        Success = 0,
        PermissionDenied = -1,
        NameNotFound = -2,
        WouldBlock = -11,
    ...
    enum class DisplayScanFormat : u32 {
        Progressive = 0,
        Interlaced = 1,
    };
    ...
    enum class GraphicsAllocatorUsage : u32 {
        SoftwareReadNever = 0,
    ...
    BIO_ENUM_BIT_OPERATORS(GraphicsAllocatorUsage, u32)

    inline constexpr u32 CalculateBytesPerPixel(ColorFormat fmt) {
        return (static_cast<u64>(fmt) >> 3) & 0x1F;
    }

    constexpr u32 BlockHeightLog2 = 4;
    constexpr u32 BlockHeight = 8 * (1 << BlockHeightLog2);

    inline constexpr u32 AlignWidth(u32 bpp, u32 width) {
        return ((width * bpp + 63) & ~63) / bpp;
    }

    inline constexpr u32 AlignHeight(u32 height) {
