...
namespace bio::ipc::client {
    ...
    constexpr u32 NoOutProcessId = 0xFFFFFFFF;
    ...
    inline void ParseCommandResponse(CommandContext &ctx) {
        auto tls = os::GetThreadLocalStorage<u8>();
        auto header = reinterpret_cast<CommandHeader*>(tls);
        tls += sizeof(CommandHeader);
    ...
    inline Result ParseRequestCommandResponse(CommandContext &ctx) {
        auto tls = os::GetThreadLocalStorage<u8>();
        ParseCommandResponse(ctx);
    ...
    inline Result ParseControlCommandResponse(CommandContext &ctx) {
        auto tls = os::GetThreadLocalStorage<u8>();
        ParseCommandResponse(ctx);
        auto data_offset = GetAlignedDataOffset(ctx.out.data_words_offset, tls);
    ...
    inline void InitializeCloseCommand(CommandContext &ctx) {
        InitializeCommand(ctx, CommandType::Close, 0);
    }
